...
{
    //PUBLIC
    const my_dac_cal_t default_dac_cal = { 1, 0, 1, 0 };
    ...
    /// @brief Get DAC calibration coefficients (k,b). Usually b = 0 and k is calculated from datasheet and circuit parameters
    /// (see default value calculation in variant params.cpp, mainly it depends on DAC resoultion, reference voltage and heater amplifier).
    /// DAC is not calibrated per se, because its specs by far superseed that of ESP32's ADC. These coefficients only provide N(V) mapping,
    /// where N is DAC code (integer) and V is desired heater amplifier output voltage.
    /// @return Tuple (k,b), usually b=0
    const my_dac_cal_t* get_dac_cal()
    {
        return &(storage.dac_cal);
    }
    /// @brief Set DAC calibration coefficients (k,b). Usually b = 0 and k is calculated from datasheet and circuit parameters
    /// (see default value calculation in variant params.cpp, mainly it depends on DAC resoultion, reference voltage and heater amplifier).
    /// DAC is not calibrated per se, because its specs by far superseed that of ESP32's ADC. These coefficients only provide N(V) mapping,
    /// where N is DAC code (integer) and V is desired heater amplifier output voltage.
    /// @param c Tuple (k,b), usually b=0
    void set_dac_cal(my_dac_cal_t* c)
    {
        storage.dac_cal = *c;
    }
    /// @brief DAC max range soft limit
    /// @return Volts 
    float get_dac_soft_sentinel()
    {
        return storage.dac_soft_sentinel;
    }
    /// @brief DAC max range soft limit
    /// @param v Volts
    void set_dac_soft_sentinel(float v)
    {
        storage.dac_soft_sentinel = v;
    }
    ...
    const uint8_t* get_nvs_dump(size_t* len)
    {
        *len = sizeof(storage);
